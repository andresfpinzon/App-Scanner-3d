import React, {
  ...
  import { engineTwoServices } from "../services/engineTwo.services"; // Nuevo servicio
  import { engineThreeServices } from "../services/engineThree.services"; // Nuevo servicio
  ...
  const EscanerScreen = () => {
    ...
    const colors = useMemo(
      () => ({
        ...
        motorButton: isDarkMode ? "#4a1e8a" : "#4a1e8a",
      }),
      [isDarkMode, theme]
    );
    ...
    // Función modificada para iniciar escaneo con motores 2 y 3
    const iniciarEscaneo = async () => {
        ...
        try {
          ...
          // 3. Esperar a que termine el escaneo (podrías usar websockets o polling)
          // Esta es una implementación simple con polling
          let escaneoCompleto = false;
          let intentos = 0;
          const maxIntentos = 10; // 10 intentos con 1 segundo entre cada uno = 10 segundos máximo
          ...
          // En caso de error, asegurarse de detener los motores
          try {
            await engineTwoServices.stopEngine();
            await engineThreeServices.stopEngine();
          } catch (e) {
